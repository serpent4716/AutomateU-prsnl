...
import extraction  # Local module for PDF text extraction
...
def summarize_experiment(query: str, num_statements_preference: str) -> dict:
    ...
    problem_statement_instruction = "Generate a JSON list of relevant problem_statements (each starting with an action verb like 'To measure' or 'To analyze')."
    ...
    - "theory": A comprehensive and well-structured theory section of approximately 3-4 paragraphs. Explain the core concepts, relevant algorithms, and any necessary mathematical formulas. Use clear, academic language.
...
def create_document(output_path, details, num_problems, selected_fields):
    ...
    if any(f in selected_fields for f in ['Conclusion', 'References']):
        ...
        if 'Conclusion' in selected_fields:
            ...
            concl_row[0].text = "CONCLUSION: " + details.get("{{conclusion}}", "")
        ...
        if 'References' in selected_fields:
            ...
            if references:
                ref_text += "\n".join([f"- {ref['title']}: {ref.get('uri', ref.get('url', 'N/A'))}" for ref in references])
            else:
                ref_text += "No references generated."
            ref_row[0].text = ref_text
